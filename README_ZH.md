...
本项目把 `enable-1m-context` 打包成一个可分享、可通过 CC Switch 安装的 Codex skill。调用后，它会运行内置脚本，完成这些操作：
...
CC Switch 支持从 GitHub 仓库安装 Skills。操作步骤：
...
CC Switch 会把 skill 安装到对应应用目录，例如 Codex 的目录通常是 `~/.codex/skills/`。
