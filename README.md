...
This project packages the `enable-1m-context` skill for Codex and CC Switch users. When invoked, the skill runs a bundled script that:
...
CC Switch supports installing Skills from GitHub repositories. In CC Switch:
...
CC Switch installs skills to app-specific directories such as `~/.codex/skills/` for Codex.
...
```text
skills/enable-1m-context  ->  ~/.codex/skills/enable-1m-context
```
